...
@Component({
  ...
  template: `
    ...
    <div [style.font-size]="'11px'"
         ...
         [style.margin-bottom]="'var(--space-2)'">Övningar · {{ rows().length }} st
    </div>
    ...
    <div [style.border]="'1px dashed var(--color-divider)'"
         ...
         (click)="showAddPicker.set(true)">+ Lägg till övning
    </div>

    <div [style.display]="'flex'" [style.flex-direction]="'column'" [style.gap]="'var(--space-2)'">
      <button type="button" class="btn btn-primary btn-block mob-btn" (click)="save()">Spara plan
      </button>
      ...
      <div class="dialog-backdrop" (click)="showAddPicker.set(false)">
        <div class="dialog"
            ...
            <div [style.display]="'flex'"
              ...
              @for (ex of availableExercises(); track ex.id) {
                <button type="button"
                  ...
                  <span [style.color]="'var(--color-neutral-500)'"
                        [style.font-size]="'12px'">{{ ex.source }}</span>
                </button>
              }
              @if (availableExercises().length === 0) {
                <div [style.font-size]="'12px'"
                     ...
                     [style.text-align]="'center'"
                     [style.padding]="'var(--space-4)'">Alla övningar är redan tillagda, eller inga
                  övningar finns.
                </div>
              }
            </div>
          ...
          <div class="dialog-actions">
            <button type="button"
                    class="btn btn-secondary mob-btn"
                    (click)="showAddPicker.set(false)">Klar
            </button>
          </div>
        </div>
      </div>
